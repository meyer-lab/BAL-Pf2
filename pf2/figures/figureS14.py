...
def score(X: anndata.AnnData,  genes):
    """Scanpy average gene score for all cells"""
    X = sc.tl.score_genes(adata=X, gene_list=genes, copy=True, use_raw=False)

    return X


def plot_score(X: anndata.AnnData, ax, cellType: str = "Label"):
    """Plots average score  across cell types and patient status"""
    df = pd.DataFrame({"Score": X.obs["score"].values})
    df["Condition"] = X.obs["sample_id"].values
    df["Label"] = X.obs[cellType].values
    df_mean_score = (
        df.groupby(["Label", "Condition"], observed=False)
        .mean()
        .reset_index()
        .sort_values(["Label", "Condition"])
    )
    df_count = (
        df.groupby(["Label", "Condition"], observed=False)
        .size()
