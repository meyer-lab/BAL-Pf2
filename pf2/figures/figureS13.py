...
def makeFigure():
    ...
    # print(cell_comp_df)
    meta_df = condition_factors_meta(X)
    print(meta_df)
    ...
    cell_comp_df["patient_category"] = meta_df["patient_category"].values
    # cell_comp_df = move_index_to_column(cell_comp_df)
    print(cell_comp_df)
    # _, meta_df = condition_factors_meta(X)
    
    # print(cell_comp_df[meta_df.index, :])
    # a
    cell_comp_df = cell_comp_df.loc[cell_comp_df["patient_category"] == "COVID-19"].drop(columns=["patient_category"])
    # meta_df = meta_df.loc[meta_df["patient_category"] != "COVID-19"]
     
    pearson_df = pd.DataFrame(
        columns=cell_comp_df.columns,
        index=cell_comp_df.columns,
        dtype=float
    )
    
    for row in cell_comp_df.columns:
        for column in cell_comp_df.columns:
            two_meta_df = cell_comp_df[[row, column]]
            result = pearsonr(
    ...
    sns.heatmap(
        ...
        vmin=0,
        vmax=.05,
        xticklabels=cell_comp_df.columns,
