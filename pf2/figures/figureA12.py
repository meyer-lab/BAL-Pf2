...
def makeFigure():
    ...
    celltype_count_perc_df = pd.concat([celltype_count_perc_df_1, celltype_count_perc_df_2, celltype_count_perc_df_3], axis=0)
    
    print(celltype_count_perc_df)
    ...
    # plot_cell_count(X[(X.obs[f"Cmp{cmp1}"] == True) & (X.obs["Both"] == False)], ax[3])
    # ax[3].set(title=f"Both - Threshold: {threshold}")
    # plot_cell_count(X[(X.obs[f"Cmp{cmp2}"] == True) & (X.obs["Both"] == False)], ax[4])
    # ax[4].set(title=f"Cmp{cmp2} - Threshold: {threshold}")
    # plot_cell_count(X[X.obs["Both"] == True], ax[5])
    # ax[5].set(title=f"Both - Threshold: {threshold}")
