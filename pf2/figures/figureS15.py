...
def makeFigure():
    ...
    X = score(X, ["MS4A1", "CD19", "BANK1"], "B_score")
    X = score(X, ["LILRA4", "TPM2", "PLD4"], "pDC_score")
    
    print(X)
    plot_avegene_scatter_cmps(X, ["B_score", "pDC_score"], cmp1, cmp2, ax[0])
...
def plot_avegene_scatter_cmps(
    ...
    gene1_data = X.obs[gene1].to_frame()
    gene2_data = X.obs[gene2].to_frame()
...
def score(X: anndata.AnnData,  genes, score_name):
    """Scanpy average gene score for all cells"""
    X = sc.tl.score_genes(adata=X, gene_list=genes, copy=True, score_name=score_name, use_raw=False)
