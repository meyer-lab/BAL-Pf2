...
def makeFigure():
    ...
    print(meta)
    accuracies, (covid_svm, coef1, nc_svm, coef2) = predict_mortality(patient_factor, meta)
    
    cmp_col = [i for i in range(1, patient_factor.shape[1] + 1)]

    df = pd.DataFrame({"Component": cmp_col, "Weight": coef1})
    ...
    df = pd.DataFrame({"Component": cmp_col, "Weight": coef2})
    ...
    ax[0].set(title="Linear SVM: C19")
    ax[1].set(title="Linear SVM: nC19")
